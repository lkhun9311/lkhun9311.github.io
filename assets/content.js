      ...
      items: [
        {
          ...
          desc_ko: "상위 인증 포털이 500을 내면서 Session 만료를 밀어 보냈고 사용자가 조용히 튕겼습니다. 뒤이어 나온 처방 「검증이 안 되면 로그아웃」은 계속 쓰고 있는 사용자만 쫓아냅니다. Polling을 브라우저가 돌리므로 위험이 진짜인 순간에 오히려 멈추기 때문입니다.",
        ...
        },
        {
          ...
          desc_ko: "Snapshot 스케줄러의 실행 이력을 화면에 붙였는데 대상이 사라져 정지한 실행은 이력이 한 줄도 남지 않았습니다. 조회 실패를 대상 없음으로 읽어 일시 오류가 영구 정지가 되고 있었습니다.",
        ...
        },
        {
          ...
          source: "company", sourceLabel: "Company work",
          title_ko: "Snapshot 스케줄러가 매번 실패했는데 화면은 STARTED였습니다",
          desc_ko: "프로젝트 Snapshot 쿼터가 차면 스케줄러가 매 실행 실패하는데 화면은 STARTED였습니다. FAILED라는 상태값은 이미 있었고 그 값을 쓰는 코드가 없었습니다. 그리고 쿼터가 찬 그 순간에 코드가 이전 백업을 먼저 지우고 있었습니다.",
        ...
        },
        {
          ...
          source: "company", sourceLabel: "Company work",
          title_ko: "2,255 커밋을 건너뛴 이관에서 절차서 세 편이 틀렸습니다",
          desc_ko: "2,255 커밋을 건너뛰는 버전 이관. 문서 추론으로는 어디서 깨질지 알 수 없어 격리 환경에 옛 버전을 세우고 한 번 깨뜨려 봤습니다. 그러자 제 앞선 문서 세 편의 전제가 틀렸다는 게 나왔습니다.",
        ...
        },
        {
          ...
          desc_ko: "스위치 장애로 살아 있는 컴퓨트 노드가 장애로 판정돼 복구가 대량 실행됐습니다. 감지를 정교하게 만드는 대신 신호에 자격을 매겼고 시나리오 여덟 중 일곱이 보류로 남았습니다.",
        ...
        },
        {
          ...
          source: "company", sourceLabel: "Company work",
          title_ko: "빈 목록이 노드가 0대인지 조회가 실패한 것인지 구분하지 못했습니다",
        ...
        },
        {
          ...
          source: "side", sourceLabel: "Personal project",
          title_ko: "kubelet을 멈췄더니 46s 만에 노드가 격리됐습니다",
          desc_ko: "GPU 노드의 「Ready」가 무엇에 대한 주장이어야 하는지를 CRD로 명시했습니다. 관측을 먼저 집행을 나중에 두 단계로 만들었고 cordon·유예 기간·결함 신호 트리거·자동 drain 네 가지는 미룬 것이 아니라 거절했습니다. kind 워커의 kubelet을 멈추자 Ready → Quarantine → Ready가 0s·46s·48초에 나왔고 그 46초가 컨트롤러가 아니라는 것은 측정이 아니라 설계에서 읽은 것이라고 적었습니다.",
          date: "2026-09", dateLabel: "Sep 2026",
    ...
    notes: {
      ...
      items: [
        ...
        },
        {
          ...
          desc_ko: "ConcurrentHashMap이 키가 속한 Bucket 하나만 잠그는 방식. computeIfAbsent의 매핑 함수가 그 잠금 안에서 돌기 때문에 거기서 외부 호출을 하면 왕복 시간 내내 버킷이 잠깁니다.",
